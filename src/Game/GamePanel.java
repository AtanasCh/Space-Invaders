...
public class GamePanel extends JPanel {
    ...
    // Size of game window and frame rate
    ...
    // Array Lists
    private ArrayList<Ship> lifeList = new ArrayList<Ship>();
...
// Used in the Enemy class to help with the draw method for the boss
    ...
    public final void setupGame() {
        ...
        if (level != 3 && level != 6 && level != 9 && level != 12) {
            ...
            for (int row = 0; row < 6; row++) {
                ...
                for (int column = 0; column < 5; column++) {
                    enemy = new Enemy((20 + (row * 100)), (20 + (column * 60)), level, 0, column, null, 40, 40); // Enemy speed increases each level
                    enemyList.add(enemy);
                }
            }
        }
        ...
        if (level == 1) {
            JOptionPane.showMessageDialog(null, "Welcome to Space Invaders!\n\nTHINGS TO KNOW:\n\n- Use left/right arrow keys to move\n- Press spacebar to shoot\n- The enemies get faster every level"
                    + "\n- Alien Boss every 3 levels\n- A SS logo will appear randomly\n- Shoot it for extra points!\n- Press R to reset high score");
        }
    ...
    public void paint(Graphics g) {
        ...
        // Draws a bullet on space bar press
        ...
        // Draws the life counter display
        g.setColor(Color.WHITE);
        ...
        // Draws the hi-score display
        g.setColor(Color.WHITE);
        g.drawString("Highscore: " + highScore, 440, 20);

        // Draws a health display for boss level in the middle of the screen
        if (level == 3 || level == 6 || level == 9 || level == 12) {
            g.setColor(Color.WHITE);
    ...
    public void updateGameState() { // int frameNumber
        ...
        if (controller.getKeyStatus(82)) { // r
            int answer = JOptionPane.showConfirmDialog(null, "Would you like to reset the HI-SCORE?", "RESET HI-SCORE", 0);
            controller.resetController();
            if (answer == 0) {
                try {
        ...
        for (int index = 0; index < beamList.size(); index++) {
            if (beamList.get(index).isColliding(playerShip)) { // if collision is detected
                beamList.remove(index);
                lifeList.remove(lifeList.size() - 1); // Removes life 
            }
        }
        ...
        for (int input = 0; input < enemyList.size(); input++) {
            ...
            // If aliens exceed this X Position, the level is reset and the player loses a life
...
// Constructor for the game panel      
    ...
    public void start() {
        ...
        gameTimer = new Timer(1000 / framesPerSecond, new ActionListener() {
           ...
           // private int frameNumber = 0; // Tracks the number of frames that have been produced.
